...
`PREP-05` is complete in synchronized Russian and English versions. The accepted Russian source in `docs/ru/project/PREP-05-decision-process.md` uses the linked artifact classes `Q / RSCH / REQ / DES / ADR / SPEC`. Each language tree contains a living `project/open-questions.md` index and mirrored question cards under `project/questions/`. After PREP-05, a new material question or process change begins with a GitHub Issue; the Issue is an entry point, not an accepted decision. Technical research under an approved program has not yet begun.
