...
`USE-0001` is the first real use-case card and exists in synchronized Russian and English versions with `Status: REVIEW` and `Disposition: CANDIDATE`. The next substantive steps are expansion of the initial `USE` set, preparation and approval of the initial systematic research plan, and only then the start of `RSCH` work. Technical research has not yet begun. Do not invent a research queue or reserve `RSCH` identifiers before the initial use cases and an approved plan exist.
...
A `USE` explicitly considers its operational profile, lifetime and verification horizon, impact of incorrect or divergent representations, adversarial or misuse incentives, and structured-data exposure and data minimization. Unknown values may remain explicit and later become `RSCH` topics; they must not be hidden assumptions. Materially different operational or risk profiles may justify separate `USE` cards even when the scenarios appear similar.
